...
set gaia [lindex $::ssk::planets_list 2]
...
ssk::pos_kepler $date $p_list bodies_larr
    ...
    append results "<p>for today: $date</p>\n\n"
...
if { 0 } {
    ...
    foreach day $day_list {
        ...
        ssk::pos_kepler $day $gaia earth_larr
        set x [lindex $earth_larr($gaia) 0]
        set y [lindex $earth_larr($gaia) 1]
        set z [lindex $earth_larr($gaia) 2]
        # 149497870.7 km/AU
...
if { 1 } {
    ...
    append results "<table>"
    append results "<tr>"
    append results "<td>date</td><td>x (AU)</td><td>y (AU)</td><td>z (AU)</td>"
    append results "<td>r (AU)</td>"
    ...
    while { $t < $ss } {
        ...
        # arctan (z / r_of_xy) = angle in radians
        set r_xy [expr { sqrt( pow( $y , 2. ) + pow( $x , 2. ) ) } ]
        set r_xyz [expr { sqrt ( pow( $x , 2. ) + pow( $y , 2. ) + pow( $z , 2. ) ) } ]
        set z_rad [expr { atan( $z / $r_xy ) } ]
        set z_deg [expr { $z_rad * $::ssk::180perpi } ]
        # format
        set f "%.5G"
        ...
        set y [format $f $y]
        set z [format $f $z]
        set z_km [format $f $z_km]
        set z_rad [format $f $z_rad]
        set z_deg [format $f $z_deg]
        set r_xyz [format $f $r_xyz]
        append results "<tr><td>${t_yyyymmdd}</td><td>${x}</td><td>${y}</td><td>${z}</td>"
        append results "<td>${r_xyz}</td>"
        append results "<td>${z_km}</td><td>${z_rad}</td><td>${z_deg}</td></tr>"
        set t [expr { $t + $step } ]
    }
    append results "</table>"
}
